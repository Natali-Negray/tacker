...
import { DayTask, TASK_TYPE_LABELS, TASK_TYPE_BORDER_COLORS, TASK_TYPE_DOT_COLORS, TIME_SLOT_LABELS, TimeSlot } from '@/lib/types'
import { StatusBadge, nextStatus } from './StatusBadge'

interface TaskCardProps {
  task: DayTask
  onStatusChange: (id: string, newStatus: DayTask['status']) => void
  onEdit: (task: DayTask) => void
  onDelete: (id: string) => void
  onMove: (task: DayTask) => void
}

export function TaskCard({ task, onStatusChange, onEdit, onDelete, onMove }: TaskCardProps) {
  const timeLabel = task.timeType === 'fixed'
    ? task.timeValue
    : TIME_SLOT_LABELS[task.timeValue as TimeSlot] ?? task.timeValue
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 mt-1">
          ...
          <span className="text-xs text-gray-400">
            {TASK_TYPE_LABELS[task.type]} · {timeLabel}
          </span>
          {task.movedFrom && (
            <span className="text-xs text-orange-400">· перенесено</span>
          )}
        </div>
      </div>
